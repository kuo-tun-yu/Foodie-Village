...
#設定參數
xgb.params = list(
  #生成新樹時的欄抽樣比例，越高代表每棵樹使用的欄就越多，增加小樹的複雜度
  colsample_bytree = 0.5,                    
  
  subsample = 0.5,  
  #subsample是隨機採樣訓練樣本(防止Overfitting)              
  booster = "gbtree",
  #max_depth是樹的最大深度，越高模型複雜度(小心Overfitting)
  max_depth = 2,           
  #boosting會增加被分錯的數據權重，而此參數是讓權重不會增加的那麼快，因此越大會讓模型愈保守
  eta = 0.04, #eta(或用'mae'也可)是學習率
  
  #eval_metric是驗證資料評估指標 
  #'auc'曲線下面積
  #"rmse"像是回歸的均方差
  #'map' Mean average precision 平均準確度
  #'logloss' 對數損失，負概似估計
  eval_metric = "rmse",            
  objective = "reg:linear", 
  #objective要看Y，reg:linear是對Y回歸 輸出數字
  #'reg:logistic' 是羅吉斯回歸 輸出類別(1,0)，
  #'binary:logistic' 是羅吉斯回歸 輸出機率
  #'multi:softmax'   多分類的問题
  
  gamma = 0)
  #gamma越大，模型會越保守，相對的模型複雜度比較低
